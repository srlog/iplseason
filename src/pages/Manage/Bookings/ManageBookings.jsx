...
import { getAllBookings, deleteBooking, updateBooking } from '../../../service/bookings.api.service';
...
const Bookings = () => {
  ...
  // Group bookings by match id
  const groupedBookings = bookings.reduce((acc, booking) => {
    const matchId = booking.match.id;
    if (!acc[matchId]) {
      acc[matchId] = [];
    }
    acc[matchId].push(booking);
    return acc;
  }, {});
  ...
  return (
    <div className="bookings-container">
      ...
      {Object.keys(groupedBookings).length === 0 ? (
        ...
        Object.entries(groupedBookings).map(([matchId, group]) => (
          <div key={matchId} className="match-group">
            <div className="match-group-header">
              <div className="match-info">
                <div className="team-info">
                  <img
                    src={group[0].match.hometeam.teamlogo}
                    alt={group[0].match.hometeam.teamname}
                    className="team-logo"
                  />
                  <span>{group[0].match.hometeam.teamname}</span>
                </div>
                <span className="vs">VS</span>
                <div className="team-info">
                  <img
                    src={group[0].match.awayteam.teamlogo}
                    alt={group[0].match.awayteam.teamname}
                    className="team-logo"
                  />
                  <span>{group[0].match.awayteam.teamname}</span>
                </div>
              </div>
              <p>
                <strong>Match Date:</strong>{' '}
                {new Date(group[0].match.matchdate).toLocaleString()}
              </p>
            </div>
            <div className="booking-cards-group">
              {group.map((booking) => (
                <BookingCard
                  key={booking.id}
                  booking={booking}
